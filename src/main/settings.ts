...
export class SettingsStore {
  ...
  async load(): Promise<AppSettings> {
    try {
      ...
      this.settings = {
        uploadsEnabled: saved.uploadsEnabled ?? fallback.uploadsEnabled,
        descriptDestinationRoot: saved.descriptDestinationRoot ?? fallback.descriptDestinationRoot,
        recordingTimezone: saved.recordingTimezone ?? fallback.recordingTimezone,
        recordingDateFormat: normalizeDateFormat(saved.recordingDateFormat ?? fallback.recordingDateFormat),
        recordingsDirectory: saved.recordingsDirectory ?? null,
        reconciliationDirectory: saved.reconciliationDirectory ?? null,
        obsHost: saved.obsHost ?? fallback.obsHost,
        obsPort: saved.obsPort ?? fallback.obsPort,
        recorderType: 'obs',
      }
    } catch {
